...
def logical_or(img_one, img_two):
    ...
    output = io.BytesIO()
    ImageChops.logical_or(img_one_resized, img_two_resized).save(output, format='PNG')
    ImageChops.logical_or(img_one_resized, img_two_resized).save(r'C:\Users\Luke\Downloads\PIL testing\trster.png',
                                                                 format='PNG')
    output.seek(0)
    return output
